...
/**
 * api/db.js — Universal API handler for DentalPro
 * Routes:
 *   GET  /api/db?t=clients              → list all
 *   POST /api/db?t=clients              → insert
 *   PATCH /api/db?t=clients&id=xxx      → update
 *   DELETE /api/db?t=clients&id=xxx     → delete
 *   GET  /api/db?t=_ping               → test connection
 *   GET  /api/db?t=_setup              → create tables
 */
...
module.exports = async function (req, res) {
  ...
  if (!process.env.NEON_DATABASE_URL) {
    return res.status(500).json({
      error: "NEON_DATABASE_URL is not set. Add it in Vercel → Settings → Environment Variables."
    });
  }
    ...
    try {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS produits (
          ...
          "prixHT" NUMERIC DEFAULT 0,
          "prixTTC" NUMERIC DEFAULT 0,
          stock INTEGER DEFAULT 0,
          "stockTheorique" INTEGER DEFAULT 0,
          "stockReel" INTEGER DEFAULT 0,
          "qteCdeClient" INTEGER DEFAULT 0,
          "qteCdeFourn" INTEGER DEFAULT 0,
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS bons_livraison (
          ...
          notes TEXT DEFAULT '',
          motif TEXT DEFAULT '',
          "tvaActive" BOOLEAN DEFAULT TRUE,
          "prixDirect" NUMERIC DEFAULT 0,
          created_at TIMESTAMPTZ DEFAULT NOW()
        )`;
